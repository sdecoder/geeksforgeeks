...
struct MaxHeap* createMaxHeap(int capacity) {
  ...
  maxHeap->array = (struct LNode**)malloc(maxHeap->capacity * sizeof(struct LNode*));
  return maxHeap;
}
...
struct MinHeap* createMinHeap(int capacity) {
  ...
  minHeap->array = (struct LNode**)malloc(minHeap->capacity * sizeof(struct LNode*));
  return minHeap;
}
...
struct MyDS* createMyDS(int capacity) {
  struct MyDS* myDS = (struct MyDS*)malloc(sizeof(struct MyDS));
...
void minHeapify(struct MinHeap* minHeap, int index) {
  ...
  if (minHeap->array[left] && left < minHeap->size
      && minHeap->array[left]->data < minHeap->array[smallest]->data)
    smallest = left;

  if (minHeap->array[right] && right < minHeap->size
  ...
  if (smallest != index) {
    ...
    swapLNode(&minHeap->array[smallest], &minHeap->array[index]);
...
void maxHeapify(struct MaxHeap* maxHeap, int index) {
  ...
  if (maxHeap->array[left] && left < maxHeap->size
      && maxHeap->array[left]->data > maxHeap->array[largest]->data)
    largest = left;

  if (maxHeap->array[right] && right < maxHeap->size
  ...
  if (largest != index) {
    ...
    // of List nodes
    swapData(
        &maxHeap->array[largest]->maxHeapIndex, &maxHeap->array[index]->maxHeapIndex);

    // Now swap pointers to List nodes
    swapLNode(&maxHeap->array[largest], &maxHeap->array[index]);
...
void insertMinHeap(struct MinHeap* minHeap, struct LNode* temp) {
  if (isMinHeapFull(minHeap)) return;
...
void insertMaxHeap(struct MaxHeap* maxHeap, struct LNode* temp) {
  if (isMaxHeapFull(maxHeap)) return;
...
int findMin(struct MyDS* myDS) {
  if (isMinHeapEmpty(myDS->minHeap)) return INT_MAX;
...
int findMax(struct MyDS* myDS) {
  if (isMaxHeapEmpty(myDS->maxHeap)) return INT_MIN;
...
void deleteMin(struct MyDS* myDS) {
  ...
  if (isMinHeapEmpty(minHeap)) return;
...
void Delete(struct MyDS* myDS, int item) {
  ...
  if (isListEmpty(myDS->list)) return;

  // search the node in List
  struct LNode* temp = myDS->list->head;
  while (temp && temp->data != item) temp = temp->next;

  // if item not found
  if (!temp || temp && temp->data != item) return;
