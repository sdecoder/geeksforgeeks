...
unsigned long long int modPow(
    unsigned long long int base, unsigned long long int exponent) {
...
void computePrefixHash(
    string str, int n, unsigned long long int prefix[], unsigned long long int power[]) {
  prefix[0] = 0;
  prefix[1] = str[0];

  for (int i = 2; i <= n; i++)
    prefix[i] =
        (prefix[i - 1] % MOD + (str[i - 1] % MOD * power[i - 1] % MOD) % MOD) % MOD;
...
void computeSuffixHash(
    string str, int n, unsigned long long int suffix[], unsigned long long int power[]) {
  suffix[0] = 0;
  suffix[1] = str[n - 1];
...
void queryResults(string str, Query q[], int m, int n, unsigned long long int prefix[],
    unsigned long long int suffix[], unsigned long long int power[]) {
  for (int i = 0; i <= m - 1; i++) {
    int L = q[i].L;
    int R = q[i].R;
    ...
    unsigned long long reverse_hash_LR = ((suffix[n - L] - suffix[n - R - 1] + MOD) % MOD
...
void computePowers(unsigned long long int power[], int n) {
  ...
  for (int i = 1; i <= n; i++) power[i] = (power[i - 1] % MOD * p % MOD) % MOD;
