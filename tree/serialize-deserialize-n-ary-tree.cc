...
Node* newNode(char key) {
  Node* temp = new Node;
  temp->key = key;
  for (int i = 0; i < N; i++) temp->child[i] = NULL;
  return temp;
}
...
void serialize(Node* root, FILE* fp) {
  ...
  for (int i = 0; i < N && root->child[i]; i++) serialize(root->child[i], fp);
...
int deSerialize(Node*& root, FILE* fp) {
  ...
  char val;
  if (!fscanf(fp, "%c ", &val) || val == MARKER) return 1;

  // Else create node with this item and recur for children
  root = newNode(val);
  for (int i = 0; i < N; i++)
    if (deSerialize(root->child[i], fp)) break;
...
void traverse(Node* root) {
  if (root) {
    printf("%c ", root->key);
    for (int i = 0; i < N; i++) traverse(root->child[i]);
  }
}
