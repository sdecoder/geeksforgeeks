...
void populateAux(int* aux, Queue* queue0, Queue* queue1, Queue* queue2, int* top) {
  ...
  while (!isEmpty(queue0)) aux[(*top)++] = Dequeue(queue0);

  // Put all items of second queue in aux[]
  while (!isEmpty(queue1)) aux[(*top)++] = Dequeue(queue1);

  // Put all items of third queue in aux[]
  while (!isEmpty(queue2)) aux[(*top)++] = Dequeue(queue2);
}
...
int findMaxMultupleOf3(int* arr, int size) {
  ...
  if ((sum % 3) == 1) {
    ...
    if (!isEmpty(queue1)) Dequeue(queue1);
  ...
  // Step 4.3: The sum produces remainder 2
    ...
    if (!isEmpty(queue2)) Dequeue(queue2);
...
int main() {
  ...
  if (findMaxMultupleOf3(arr, size) == 0) printf("Not Possible");
